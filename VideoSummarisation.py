...
def detect_shot_boundaries(video_path, threshold):
    ...
    for i in range(1, frame_count):
        ...
        if res:
            print(frame_number)
            frame_number+=1
            diff = cv2.absdiff(curr_frame, prev_frame) #Mean Absolute Difference(MAD) is being calculated
            diff_mean = diff.mean()

            if diff_mean > threshold:
                # Add current frame to unique frames list
    ...
    fourcc = cv2.VideoWriter_fourcc(*"XVID") # This function is used to define a codec - codec is a compressor and decompressor of a video 
